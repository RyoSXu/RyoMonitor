...
RyoMonitor 是一个轻量的自托管 VPS 监控面板，支持明暗主题、密码登录，并且不需要前端构建步骤。
...
app/assets/*.png            主屏 / PWA 图标（apple-touch-icon、192、512）
app/assets/site.webmanifest Web 应用清单（添加到主屏元数据）
...
docs/screenshot-*.png       看板截图（浅色 / 深色主题）
.env.example                环境变量示例
```
...
| `MON_AUTH_PASSWORD_HASH` / `MON_AUTH_SECRET` | 登录密码哈希与会话签名密钥。 |
| `MON_AUTH_TRUST_PROXY` | 置 `1` 时信任前置反代 / SSO，跳过内置登录（此时 `HASH`/`SECRET` 可留空）。 |
...
如果鉴权已经在 RyoMonitor 前面完成（例如 Caddy `forward_auth`、SSO 门户，或处于内网），可设置：

```bash
MON_AUTH_TRUST_PROXY=1
```

此时 RyoMonitor 会跳过自带登录页，直接提供看板。仅在上游已限制访问时才开启。
